/*    JavaScript 7th Edition
     ...
     Rodriguez Ramon, August 27, 2023, IST239-W01, Hands-on Project 2-5
 */
 
// Add event handlers for each calculator button sending that buttons value to the runCalculator() function
document.getElementById("button0").onclick = function() {
   let a = document.getElementById("calcWindow").value;

   //if the text area is empty then show number 0
   if(a == "")
   {
      document.getElementById("calcWindow").value = 0;
   }
   else if(a != "") //if the text area is not empty then concat the number 0 to the value of the text area
   {
      document.getElementById("calcWindow").value += 0;
   }
}
